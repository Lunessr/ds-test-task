...
export class StartingMigration1670532039775 implements MigrationInterface {
  name = 'StartingMigration1670532039775';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`full_name\` varchar(100) COLLATE "utf8mb4_unicode_ci" NOT NULL, \`birth_date\` date NOT NULL, \`email\` varchar(200) NOT NULL, \`uaid\` varchar(24) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`activity_status\` int NULL, UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), UNIQUE INDEX \`IDX_c5d6c89440fe1b78b20183e806\` (\`uaid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`activity_status_code\` (\`id\` int NOT NULL AUTO_INCREMENT, \`user_activity_status\` enum ('active', 'suspended', 'archived') NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD CONSTRAINT \`FK_26ced29f734efa118b81106a241\` FOREIGN KEY (\`activity_status\`) REFERENCES \`activity_status_code\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    ...
    await queryRunner.query(
      `INSERT INTO activity_status_code (user_activity_status) VALUES ('active')`,
    );
    await queryRunner.query(
      `INSERT INTO activity_status_code (user_activity_status) VALUES ('suspended')`,
    );
    await queryRunner.query(
      `INSERT INTO activity_status_code (user_activity_status) VALUES ('archived')`,
    );
    await queryRunner.query(
      `CREATE PROCEDURE getUserStatusCodeByStatus(IN status CHAR(100))
                      BEGIN
                          SELECT * FROM activity_status_code WHERE user_activity_status=status;
                      END `,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE PROCEDURE getAllUsers() 
                BEGIN
                    SELECT *  FROM user
                    LEFT JOIN activity_status_code
                    ON user.activity_status = activity_status_code.user_activity_status
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TABLE \`activity_status_code\``);
    await queryRunner.query(
